...
import { 
  Users, Wallet, CheckCircle, XCircle, Plus, Trash2, Edit3, Award, LogOut, LogIn, Lock, Key, ShieldAlert, Eye, Calendar, Trophy
} from 'lucide-react';
        ...
        <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-4 font-sans">
            <div className="bg-white max-w-md w-full rounded-2xl shadow-xl p-8 space-y-6">
                <div className="text-center space-y-2">
                    ...
                    <p className="text-xs text-slate-500">Nhập Mã số học sinh để xem kết quả và hoàn thành phiếu điều tra</p>
                </div>
...
// 2. MÀN HÌNH CÁ NHÂN CỦA HỌC SINH (HIỂN THỊ VI PHẠM, KHEN THƯỞNG VÀ TỔNG ĐIỂM)
function StudentPortal({ student, onLogout }: { student: Student; onLogout: () => void }) {
    const [currentStudent] = useState<Student>(student);
    ...
    // Tính tổng điểm thi đua sau cùng
...
function TeacherDashboard({ teacherPass, onUpdatePass, onLogoutTeacher }: { teacherPass: string; onUpdatePass: (p: string) => void; onLogoutTeacher: () => void }) {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [stRes, itemRes, payRes, vioRes, comRes, grpRes] = await Promise.all([
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
            <header className="bg-indigo-700 text-white shadow-lg py-4 px-6 flex justify-between items-center">
                <div>
                                                        ...
                                                        return (
                                                            <td key={item.id} className="p-3 border-r text-center">
                                                                <button
                                                                    ...
                                                                    className={`px-3 py-1 rounded font-bold text-[11px] transition ${isPaid ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-rose-100 text-rose-700 hover:bg-rose-200'
                                                                        }`}
                                                                >
